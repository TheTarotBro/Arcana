...
function SettingsSection({spreads,saveSpreads,themeId,saveTheme,onBack}){const [view,setView]=useState("main");const [editSpread,setEditSpread]=useState(null);const [cd,setCd]=useState(null);
  ...
  return<div style={styles.section}>{cd&&<ConfirmModal message="Delete this spread?" onConfirm={()=>{saveSpreads(spreads.filter(s=>s.id!==cd));setCd(null);}} onCancel={()=>setCd(null)}/>}<BackBtn onClick={onBack}/><h2 style={styles.pageTitle}>Settings</h2>
