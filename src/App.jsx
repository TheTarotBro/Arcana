...
const PRESET_LAYOUTS = {
  ...
  s3: { // Celtic Cross — cross left, staff right
    height:330,
    cards:[
      {posId:"p1",cx:26,cy:42},           // Present — center
      {posId:"p2",cx:26,cy:42,rotate:90}, // Challenge — crossing
      {posId:"p3",cx:26,cy:72},           // Foundation — below
      {posId:"p4",cx:8,cy:42},            // Recent Past — left
      {posId:"p5",cx:26,cy:13},           // Crown — above
      {posId:"p6",cx:44,cy:42},           // Near Future — right
      {posId:"p7",cx:80,cy:86},           // Self — staff bottom
      {posId:"p8",cx:80,cy:62},           // Environment
      {posId:"p9",cx:80,cy:38},           // Hopes & Fears
      {posId:"p10",cx:80,cy:14},          // Outcome — staff top
    ]
  },
...
function autoLayout(positions) {
  ...
  return {
    ...
    cards: positions.map((p, i) => {
      ...
      const offset = (cols - itemsInRow) * cellW / 2;
      return { posId:p.id, cx:offset + col * cellW + cellW / 2, cy:((row + 0.5) / rows) * 100 };
    })
  };
}

function getSpreadLayout(spread) {
  if (PRESET_LAYOUTS[spread.id]) return PRESET_LAYOUTS[spread.id];
  return autoLayout(spread.positions);
}

function getEntryLayout(entry) {
  if (entry.spreadId && PRESET_LAYOUTS[entry.spreadId]) return PRESET_LAYOUTS[entry.spreadId];
  const match = DEFAULT_SPREADS.find(s => s.name === entry.spreadName);
  if (match && PRESET_LAYOUTS[match.id]) return PRESET_LAYOUTS[match.id];
  const positions = (entry.slots||[]).map((s,i) => ({id:s.posId||`a${i}`, label:s.label}));
  return autoLayout(positions);
}
...
const suitColor = (card) => { if(!card||!card.suit)return"#c9a96e"; if(card.suit==="Wands")return"#e07340"; if(card.suit==="Cups")return"#5b8fc9"; if(card.suit==="Swords")return"#9fc5c5"; if(card.suit==="Pentacles")return"#6db87a"; return"#c9a96e"; };
    ...
    <div style={styles.modalOverlay}>
      <div style={styles.modalBox}>
        ...
        <div style={styles.btnRow}>
          ...
          <button style={{...styles.btnPrimary,flex:1,borderColor:"#7a3030",background:"linear-gradient(135deg,#3a1525,#2a1020)"}} onClick={onConfirm}>Delete</button>
        </div>
      </div>
    </div>
  );
...
export default function TarotApp() {
  ...
  if (!loaded) return (
    <div style={styles.loading}>
      ...
      <div style={{color:"#c9a96e",fontFamily:"Cormorant Garamond,serif",fontSize:14,letterSpacing:3}}>LOADING</div>
    </div>
  );
...
function SpreadMap({ layout, slots, activeIdx, onSelect, readOnly }) {
  ...
  return (
    <div style={{position:"relative",width:"100%",height:layout.height,marginBottom:16,borderRadius:14,background:"linear-gradient(160deg,#1a1228,#160f22)",border:`1px solid ${C.borderFaint}`,overflow:"hidden"}}>
      <div style={{position:"absolute",inset:0,opacity:0.04,backgroundImage:`radial-gradient(circle,${C.gold} 1px,transparent 1px)`,backgroundSize:"28px 28px"}} />
      {layout.cards.map((pos, li) => {
        const si = slots.findIndex(s => s.posId === pos.posId);
        const slot = si >= 0 ? slots[si] : null;
        if (!slot) return null;
        const card = slot.card ? ALL_CARDS.find(c => c.name === slot.card) : null;
        const hasCard = !!slot.card;
        const hasNotes = !!(slot.note && slot.note.trim());
        const isActive = si === activeIdx;
        const rot = pos.rotate || 0;

        let bg = C.bgInput, bc = C.borderFaint;
        if (isActive) { bg="#2e2450"; bc=C.gold; }
        else if (hasCard && hasNotes) { bg="#2a2145"; bc=C.goldDim; }
        else if (hasCard) { bg=C.bgCard; bc=suitColor(card)+"70"; }

        return (
          <button key={pos.posId} onClick={() => !readOnly && onSelect && onSelect(si)}
            style={{position:"absolute",left:`${pos.cx}%`,top:`${pos.cy}%`,
              ...
              boxShadow:isActive?`0 0 12px ${C.gold}30`:hasCard?"0 2px 8px #00000030":"none"}}>
            {hasCard
              ? <span style={{fontSize:16,color:suitColor(card),lineHeight:1,transform:rot?`rotate(-${rot}deg)`:"none"}}>{suitSymbol(card)}</span>
              : <span style={{fontSize:13,color:C.textDim,fontWeight:600,lineHeight:1}}>{li+1}</span>}
            <span style={{fontSize:7,color:hasCard?C.textMid:C.textDim,textTransform:"uppercase",letterSpacing:0.5,
              maxWidth:SLOT_W-8,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap",textAlign:"center",lineHeight:1.2,
              transform:rot?`rotate(-${rot}deg)`:"none"}}>{slot.label}</span>
            {hasCard && hasNotes && !isActive && (
              <div style={{position:"absolute",top:3,right:3,width:5,height:5,borderRadius:"50%",background:C.gold,transform:rot?`rotate(-${rot}deg)`:"none"}} />
            )}
          </button>
        );
      })}
...
function JournalSection({ entries, saveEntries, spreads }) {
  ...
  const updateEntry = (updated) => { saveEntries(entries.map(e => e.id===updated.id?updated:e)); setActiveEntry(updated); };
  ...
  return (
    <div style={styles.section}>
      ...
      <div style={styles.quickRow}>
        <QuickBtn icon="☽" label="Daily Pull" onClick={() => setView("daily")} accent="#c9a96e" />
        <QuickBtn icon="◈" label="New Reading" onClick={() => setView("newEntry")} accent="#9b7fc9" />
      </div>
...
function getStalkerCard(entries) {
  const counts = {};
  entries.forEach(e => { (e.slots||[]).forEach(s => { if(s.card) counts[s.card]=(counts[s.card]||0)+1; if(s.clarifier) counts[s.clarifier]=(counts[s.clarifier]||0)+1; }); if(e.card) counts[e.card]=(counts[e.card]||0)+1; });
...
function QuickBtn({ icon, label, onClick, accent }) {
  return <button onClick={onClick} style={{...styles.quickBtn,borderColor:accent+"60"}}><span style={{fontSize:22,color:accent}}>{icon}</span><span style={{...styles.quickBtnLabel,color:accent}}>{label}</span></button>;
}
      ...
      {alreadyPulled && !card ? (
        <div>
          <div style={styles.alreadyPulledBox}>
            <div style={styles.alreadyPulledLabel}>Today's Card</div>
            <div style={styles.alreadyPulledCard}>{existingEntry?.card}</div>
            <div style={styles.alreadyPulledDate}>pulled earlier today</div>
            {existingEntry?.note && <p style={styles.alreadyPulledNote}>{existingEntry.note}</p>}
          </div>
          <button style={{...styles.btnSecondary,width:"100%",marginTop:14}} onClick={drawAnyway}>Draw another card anyway</button>
        </div>
      ) : !card ? (
        <div style={styles.drawArea}><div style={styles.cardBack} onClick={draw}><div style={styles.cardBackInner}>✦</div><div style={styles.cardBackHint}>Tap to draw</div></div></div>
      ) : (
        <div>
          <div style={{...styles.drawnCard,borderColor:suitColor(card)}}>
            <div style={{color:suitColor(card),fontSize:28}}>{suitSymbol(card)}</div>
            <div style={styles.drawnCardName}>{card.name}</div>
            <div style={styles.drawnCardKeywords}>{card.keywords.join(" · ")}</div>
            <p style={styles.drawnCardMeaning}>{card.meaning}</p>
          </div>
          {!saved ? (
            <>
              <textarea style={styles.textarea} placeholder="Your reflections on this card today…" value={note} onChange={e=>setNote(e.target.value)} rows={4} />
              <div style={styles.btnRow}><button style={styles.btnSecondary} onClick={draw}>Draw Again</button><button style={styles.btnPrimary} onClick={save}>Save to Journal</button></div>
            </>
...
function NewEntry({ spreads, onSave, onBack }) {
  ...
  const pickSpread = (sp) => {
    setSpread(sp);
    setSlots(sp.positions.map(p => ({posId:p.id,label:p.label,meaning:p.meaning,card:"",clarifier:"",showClarifier:false,note:""})));
    setActiveSlot(0); setStep("fill");
  };
  ...
  const save = () => {
    onSave({id:Date.now().toString(),isDaily:false,date:formatDate(),dateKey:todayKey(),spreadId:spread.id,spreadName:spread.name,question,slots,note:globalNote});
  };

  if (step==="pick") return (
    <div style={styles.section}>
      <BackBtn onClick={onBack} />
      <h2 style={styles.pageTitle}>Choose a Spread</h2>
      {spreads.map(sp => (
        <button key={sp.id} style={styles.spreadPickCard} onClick={() => pickSpread(sp)}>
          <div style={styles.spreadPickName}>{sp.name}</div>
          <div style={styles.spreadPickPositions}>{sp.positions.map(p=>p.label).join(" · ")}</div>
          <div style={styles.spreadPickCount}>{sp.positions.length} {sp.positions.length===1?"card":"cards"}</div>
        </button>
      ))}
    </div>
  );
  ...
  if (step==="fill") {
    ...
    const activeCard = slot?.card ? ALL_CARDS.find(c=>c.name===slot.card) : null;

    return (
      <div style={styles.section}>
        <BackBtn onClick={() => setStep("pick")} />
        <h2 style={styles.pageTitle}>{spread.name}</h2>
        ...
        <div style={styles.mapProgress}>
          <span>{filledCount} of {slots.length} positions filled</span>
          {slots.every(s=>s.card) && <span style={{color:C.gold}}> ✦ all cards placed</span>}
        </div>

        {slot && (
          <div style={styles.slotPanel}>
            <div style={styles.slotPanelHeader}>
              <div style={styles.slotPanelNum}>{activeSlot+1}</div>
              <div><div style={styles.slotLabel}>{slot.label}</div><div style={styles.slotMeaning}>{slot.meaning}</div></div>
            </div>
            <CardSelect value={slot.card} onChange={v=>updateSlot(activeSlot,"card",v)} placeholder="Select card…" />
            {slot.card && activeCard && (
              <div style={styles.slotPanelCardInfo}>
                <span style={{color:suitColor(activeCard),marginRight:6}}>{suitSymbol(activeCard)}</span>
                <span style={{color:C.textMid,fontStyle:"italic",fontSize:12}}>{activeCard.keywords.join(" · ")}</span>
              </div>
            )}
            {slot.card && (
              <button style={styles.clarifierToggle} onClick={() => updateSlot(activeSlot,"showClarifier",!slot.showClarifier)}>
                {slot.showClarifier?"− Remove clarifier":"+ Add clarifier"}
              </button>
            )}
            {slot.showClarifier && (
              <div style={styles.clarifierArea}><div style={styles.clarifierLine} /><div style={{flex:1}}><div style={styles.clarifierLabel}>Clarifier</div><CardSelect value={slot.clarifier} onChange={v=>updateSlot(activeSlot,"clarifier",v)} placeholder="Select clarifier…" /></div></div>
            )}
            ...
            <div style={styles.slotPanelNav}>
              <button style={{...styles.slotNavBtn,...(activeSlot===0?{opacity:.3}:{})}} disabled={activeSlot===0} onClick={() => setActiveSlot(activeSlot-1)}>← {activeSlot>0?slots[activeSlot-1].label:""}</button>
              <button style={{...styles.slotNavBtn,...(activeSlot===slots.length-1?{opacity:.3}:{})}} disabled={activeSlot===slots.length-1} onClick={() => setActiveSlot(activeSlot+1)}>{activeSlot<slots.length-1?slots[activeSlot+1].label:""} →</button>
            </div>
          </div>
        )}
  ...
  return (
    <div style={styles.section}>
      <BackBtn onClick={() => setStep("fill")} />
      <h2 style={styles.pageTitle}>Overall Reflection</h2>
      <textarea style={styles.textarea} placeholder="Overall impressions, themes, feelings about this reading…" value={globalNote} onChange={e=>setGlobalNote(e.target.value)} rows={6} />
      <button style={styles.btnPrimary} onClick={save}>Save Reading ✦</button>
    </div>
  );
}
...
function EditEntry({ entry, onSave, onBack }) {
  ...
  const [dailyNote, setDailyNote] = useState(entry.isDaily?(entry.note||""):"");
  const [slots, setSlots] = useState((entry.slots||[]).map(s=>({...s})));
  const [activeSlot, setActiveSlot] = useState(0);
  ...
  if (entry.isDaily) {
    return (
      <div style={styles.section}>
        <BackBtn onClick={onBack} />
        <h2 style={styles.pageTitle}>Edit Daily Pull</h2>
        <div style={styles.editCardDisplay}><span style={{color:suitColor(ALL_CARDS.find(c=>c.name===entry.card)||{}),marginRight:8}}>{suitSymbol(ALL_CARDS.find(c=>c.name===entry.card)||{})}</span>{entry.card}</div>
        <label style={styles.fieldLabel}>Your Reflection</label>
        <textarea style={styles.textarea} value={dailyNote} onChange={e=>setDailyNote(e.target.value)} rows={5} placeholder="Your reflections…" />
        <button style={styles.btnPrimary} onClick={save}>Save Changes ✦</button>
      </div>
    );
  }

  const layout = getEntryLayout(entry);
  const slot = slots[activeSlot];

  return (
    <div style={styles.section}>
      <BackBtn onClick={onBack} />
      <h2 style={styles.pageTitle}>Edit Reading</h2>
      <div style={styles.editMeta}>{entry.spreadName} · {entry.date}</div>
      <label style={styles.fieldLabel}>Question / Intention</label>
      <input style={styles.input} placeholder="Your question or intention" value={question} onChange={e=>setQuestion(e.target.value)} />
      <SpreadMap layout={layout} slots={slots} activeIdx={activeSlot} onSelect={setActiveSlot} />
      {slot && (
        <div style={styles.slotPanel}>
          <div style={styles.slotPanelHeader}><div style={styles.slotPanelNum}>{activeSlot+1}</div><div><div style={styles.slotLabel}>{slot.label}</div><div style={styles.slotMeaning}>{slot.meaning}</div></div></div>
          <CardSelect value={slot.card} onChange={v=>updateSlot(activeSlot,"card",v)} placeholder="Select card…" />
      ...
      <label style={{...styles.fieldLabel,marginTop:12}}>Overall Reflection</label>
      <textarea style={styles.textarea} value={note} onChange={e=>setNote(e.target.value)} rows={5} placeholder="Overall impressions, themes, feelings…" />
      <button style={styles.btnPrimary} onClick={save}>Save Changes ✦</button>
    </div>
  );
}
...
function CardSelect({ value, onChange, placeholder }) {
  ...
  if (!open) return (
    <button style={{...styles.cardSelectBtn,borderColor:card?suitColor(card)+"80":"#3a3030"}} onClick={() => setOpen(true)}>
      {card?<><span style={{color:suitColor(card)}}>{suitSymbol(card)}</span> {card.name}</>:<span style={{opacity:.4}}>{placeholder}</span>}
    </button>
  );
  return (
    <div style={styles.cardSelectOpen}>
      <input autoFocus style={styles.cardSearchInput} placeholder="Search cards…" value={search} onChange={e=>setSearch(e.target.value)} />
      <div style={styles.cardList}>{filtered.map(c=><button key={c.name} style={styles.cardListItem} onClick={() => {onChange(c.name);setOpen(false);setSearch("");}}><span style={{color:suitColor(c),marginRight:6}}>{suitSymbol(c)}</span>{c.name}</button>)}</div>
      <button style={styles.cardSelectCancel} onClick={() => setOpen(false)}>Cancel</button>
    </div>
  );
}

// ── Entry Detail (read-only with visual map) ─────────────────────────
function EntryDetail({ entry, onBack, onEdit, onDelete }) {
  const [confirmDelete, setConfirmDelete] = useState(false);
  const [expandedSlot, setExpandedSlot] = useState(null);

  const hasSlots = (entry.slots||[]).length > 0 && !entry.isDaily;
  const layout = hasSlots ? getEntryLayout(entry) : null;

  return (
    <div style={styles.section}>
      {confirmDelete && <ConfirmModal message="Delete this reading permanently? This cannot be undone." onConfirm={() => {setConfirmDelete(false);onDelete();}} onCancel={() => setConfirmDelete(false)} />}
      <BackBtn onClick={onBack} />
      <div style={styles.entryDetailHeader}><h2 style={styles.pageTitle}>{entry.spreadName}</h2><div style={styles.entryDate}>{entry.date}</div></div>
      {entry.question && <div style={styles.entryQuestion}>"{entry.question}"</div>}

      {entry.isDaily && entry.card && (
        <div style={{...styles.drawnCard,borderColor:suitColor(ALL_CARDS.find(c=>c.name===entry.card)||{})}}>
          <div style={styles.drawnCardName}>{entry.card}</div>
          {entry.note && <p style={styles.drawnCardMeaning}>{entry.note}</p>}
        </div>
      )}

      {hasSlots && layout && (
        <>
          <SpreadMap layout={layout} slots={entry.slots} activeIdx={expandedSlot} onSelect={setExpandedSlot} />
          <div style={styles.mapHint}>tap a position to see details</div>
        </>
      )}

      {hasSlots && expandedSlot !== null && entry.slots[expandedSlot] && (() => {
        const slot = entry.slots[expandedSlot];
        const card = ALL_CARDS.find(c=>c.name===slot.card);
        const clarCard = ALL_CARDS.find(c=>c.name===slot.clarifier);
        return (
          <div style={styles.slotDetailExpanded}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start"}}>
              <div><div style={styles.slotLabel}>{slot.label}</div><div style={styles.slotMeaning}>{slot.meaning}</div></div>
              <button onClick={() => setExpandedSlot(null)} style={{background:"none",border:"none",color:C.textDim,cursor:"pointer",fontSize:16,padding:0}}>✕</button>
            </div>
            {card && <div style={{...styles.cardPill,borderColor:suitColor(card)+"80",color:suitColor(card),marginTop:8}}>{suitSymbol(card)} {card.name}</div>}
            ...
            {slot.note && <p style={styles.slotNote}>{slot.note}</p>}
          </div>
        );
      })()}

      {hasSlots && expandedSlot === null && (entry.slots||[]).map((slot,i) => {
        const card = ALL_CARDS.find(c=>c.name===slot.card);
        if (!card) return null;
        return (
          <div key={i} style={styles.slotCardCompact} onClick={() => setExpandedSlot(i)}>
            <span style={{color:suitColor(card),marginRight:8}}>{suitSymbol(card)}</span>
            <span style={{fontSize:12,color:C.textMid,flex:1}}><span style={{color:C.gold}}>{slot.label}:</span> {card.name}</span>
            {slot.note && <span style={{fontSize:10,color:C.goldDim}}>✎</span>}
          </div>
        );
      })}
      ...
      <div style={styles.btnRow}>
        ...
        <button style={{...styles.btnSecondary,borderColor:"#7a3030",color:"#c97070"}} onClick={() => setConfirmDelete(true)}>Delete</button>
      </div>
    </div>
  );
}
...
function SpreadBuilder({ spread, onSave, onBack }) {
  const [name, setName] = useState(spread?.name||"");
  ...
  return (
    <div style={styles.section}>
      <BackBtn onClick={onBack} />
      <h2 style={styles.pageTitle}>{spread?"Edit Spread":"New Spread"}</h2>
      ...
      {positions.length > 0 && (
        <><div style={{...styles.fieldLabel,marginBottom:8}}>Layout Preview</div><SpreadMap layout={previewLayout} slots={previewSlots} activeIdx={-1} readOnly /></>
      )}
      {positions.map((pos,i) => (
        <div key={pos.id} style={styles.posRow}>
          <div style={styles.posReorder}><button style={styles.reorderBtn} onClick={()=>moveUp(i)}>▲</button><button style={styles.reorderBtn} onClick={()=>moveDown(i)}>▼</button></div>
          <div style={{flex:1}}><input style={{...styles.input,marginBottom:4}} placeholder={`Position ${i+1} label`} value={pos.label} onChange={e=>updatePos(i,"label",e.target.value)} /><input style={styles.input} placeholder="Meaning / description" value={pos.meaning} onChange={e=>updatePos(i,"meaning",e.target.value)} /></div>
          <button style={styles.removeBtn} onClick={()=>removePos(i)}>✕</button>
        </div>
      ))}
      <button style={styles.btnSecondary} onClick={addPos}>+ Add Position</button>
      <button style={{...styles.btnPrimary,marginTop:8}} onClick={save}>Save Spread ✦</button>
    </div>
  );
}
...
function CardsSection({ cardNotes, saveCardNotes, entries }) {
  ...
  return (
    <div style={styles.section}>
      <h2 style={styles.pageTitle}>The 78 Cards</h2>
      ...
      <div style={styles.cardGrid}>{filtered.map(card=>(
        <button key={card.name} style={{...styles.cardGridItem,borderColor:suitColor(card)+"50"}} onClick={()=>{setActiveCard(card);setView("card");}}>
          <div style={{color:suitColor(card),fontSize:18,marginBottom:4}}>{suitSymbol(card)}</div>
          <div style={styles.cardGridName}>{card.name}</div>
          {counts[card.name]&&<div style={styles.cardGridCount}>{counts[card.name]}×</div>}
          {cardNotes[card.name]?.myMeaning&&<div style={styles.cardGridEdited}>✎</div>}
        </button>
      ))}</div>
    </div>
  );
...
function CardDetail({ card, notes, onSave, onBack, count }) {
  const [myKeywords, setMyKeywords] = useState(notes.myKeywords||"");
  const [myMeaning, setMyMeaning] = useState(notes.myMeaning||"");
  const [myNote, setMyNote] = useState(notes.myNote||"");
  ...
  return (
    <div style={styles.section}>
      <BackBtn onClick={onBack} />
      <div style={{...styles.cardDetailHero,borderColor:suitColor(card)}}>
        <div style={{fontSize:36,color:suitColor(card)}}>{suitSymbol(card)}</div>
        <h2 style={styles.cardDetailName}>{card.name}</h2>
        {card.num&&<div style={styles.cardDetailNum}>{card.num}</div>}
        {count>0&&<div style={styles.cardDetailCount}>Appeared {count}× in your journal</div>}
      </div>
      <div style={styles.tabRow}>
        <button style={{...styles.tabBtn,...(tab==="standard"?styles.tabBtnActive:{})}} onClick={()=>setTab("standard")}>Standard</button>
        <button style={{...styles.tabBtn,...(tab==="mine"?styles.tabBtnActive:{})}} onClick={()=>setTab("mine")}>My Notes</button>
      </div>
      ...
      {tab==="mine"&&(
        <div style={styles.cardMine}>
          <label style={styles.fieldLabel}>My Keywords</label><input style={styles.input} placeholder="Your own keywords…" value={myKeywords} onChange={e=>setMyKeywords(e.target.value)} />
          <label style={styles.fieldLabel}>My Meaning</label><textarea style={styles.textarea} placeholder="What this card means to you…" value={myMeaning} onChange={e=>setMyMeaning(e.target.value)} rows={3} />
          <label style={styles.fieldLabel}>Personal Notes</label><textarea style={styles.textarea} placeholder="Memories, readings, associations…" value={myNote} onChange={e=>setMyNote(e.target.value)} rows={4} />
          <button style={styles.btnPrimary} onClick={save}>Save Notes ✦</button>
        </div>
      )}
    </div>
  );
...
function DecksSection({ decks, saveDecks }) {
  ...
  return (
    <div style={styles.section}>
      <div style={styles.sectionHeaderRow}><h2 style={styles.pageTitle}>My Decks</h2><button style={styles.btnPrimary} onClick={()=>setView("new")}>+ Add Deck</button></div>
      {decks.length===0&&<EmptyState text="Add your first deck to start your collection" />}
      <div style={styles.deckGrid}>{decks.map(deck=>(
        <button key={deck.id} style={styles.deckCard} onClick={()=>{setActiveDeck(deck);setView("detail");}}>
          ...
          <div style={styles.deckCardName}>{deck.name}</div>{deck.acquired&&<div style={styles.deckCardDate}>{deck.acquired}</div>}
        </button>
      ))}</div>
    </div>
  );
...
function DeckDetail({ deck, onBack, onEdit, onDelete, onSave }) {
  ...
  return (
    <div style={styles.section}>
      {confirmDelete&&<ConfirmModal message={`Delete "${deck.name}"? This cannot be undone.`} onConfirm={()=>{setConfirmDelete(false);onDelete();}} onCancel={()=>setConfirmDelete(false)} />}
      <BackBtn onClick={onBack} />
      <div style={styles.deckDetailHero}>
        {deck.photo?<img src={deck.photo} alt={deck.name} style={styles.deckDetailCover}/>:<div style={styles.deckDetailCoverPlaceholder}>◎</div>}
        <h2 style={styles.pageTitle}>{deck.name}</h2>{deck.acquired&&<div style={styles.deckAcquired}>Acquired {deck.acquired}</div>}
      </div>
      ...
      <div style={styles.btnRow}><button style={styles.btnSecondary} onClick={onEdit}>Edit</button><button style={{...styles.btnSecondary,borderColor:"#7a3030",color:"#c97070"}} onClick={()=>setConfirmDelete(true)}>Delete</button></div>
    </div>
  );
}
...
function DeckForm({ deck, onSave, onBack }) {
  ...
  const [acquired, setAcquired] = useState(deck?.acquired||"");
  const [notes, setNotes] = useState(deck?.notes||"");
  const [tags, setTags] = useState(deck?.tags?.join(", ")||"");
  const [photo, setPhoto] = useState(deck?.photo||null);
  ...
  return (
    <div style={styles.section}>
      <BackBtn onClick={onBack} /><h2 style={styles.pageTitle}>{deck?"Edit Deck":"Add a Deck"}</h2>
      ...
      <button style={styles.btnPrimary} onClick={save}>Save Deck ✦</button>
    </div>
  );
}
...
function SettingsSection({ spreads, saveSpreads, onBack }) {
  ...
  const [editSpread, setEditSpread] = useState(null);
  const [confirmDelete, setConfirmDelete] = useState(null);
  ...
  return (
    <div style={styles.section}>
      ...
      <div style={styles.settingsList}>{spreads.map(sp=>(
        <div key={sp.id} style={styles.settingsRow}>
          <div style={styles.settingsRowInfo}><div style={styles.settingsRowName}>{sp.name}</div><div style={styles.settingsRowMeta}>{sp.positions.length} positions</div></div>
          <div style={styles.settingsRowActions}>
            <button style={styles.settingsRowBtn} onClick={()=>{setEditSpread(sp);setView("editSpread");}}>Edit</button>
            {!DEFAULT_SPREADS.find(d=>d.id===sp.id)&&<button style={{...styles.settingsRowBtn,color:C.rose}} onClick={()=>setConfirmDelete(sp.id)}>Delete</button>}
          </div>
        </div>
      ))}</div>
    </div>
...
function EntryCard({ entry, onClick }) {
  return (
    <button style={styles.entryCard} onClick={onClick}>
      <div style={styles.entryCardTop}><span style={styles.entryCardSpread}>{entry.spreadName}</span><span style={styles.entryCardDate}>{entry.date}</span></div>
...
// STYLES
// ══════════════════════════════════════════════════════════════════
const C = {bg:"#1c1528",bgCard:"#251d36",bgInput:"#160f22",bgHover:"#2e2445",border:"#3d2f58",borderFaint:"#2a2040",gold:"#e2b96a",goldDim:"#a07c3a",goldFaint:"#e2b96a22",text:"#ede0cc",textMid:"#b8a88a",textDim:"#7a6a82",rose:"#c97090",header:"linear-gradient(180deg,#211830 0%,#1c1528 100%)"};

const styles = {
  root:{minHeight:"100vh",background:C.bg,display:"flex",flexDirection:"column",fontFamily:"'Cormorant Garamond',Georgia,serif",color:C.text,maxWidth:480,margin:"0 auto",position:"relative"},
  loading:{minHeight:"100vh",background:C.bg,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",gap:12},
  loadingSymbol:{color:C.gold,fontSize:32},
  ...
  headerSymbol:{color:C.gold,fontSize:13},headerTitle:{fontSize:22,letterSpacing:8,color:C.text,fontWeight:400},
  ...
  navBtnActive:{color:C.gold},navIcon:{fontSize:18},navLabel:{fontSize:9,letterSpacing:2,textTransform:"uppercase"},
  section:{padding:"22px 16px 16px"},
  pageTitle:{fontFamily:"'Cormorant Garamond',Georgia,serif",fontSize:25,fontWeight:400,letterSpacing:3,color:C.text,margin:"0 0 22px",textAlign:"center"},
  ...
  stalkerBox:{background:"linear-gradient(135deg,#2a1e42,#1e1630)",border:"1px solid #6b3a7a",borderRadius:12,padding:"16px",marginBottom:22,textAlign:"center"},
  stalkerLabel:{fontSize:10,letterSpacing:3,color:C.textDim,textTransform:"uppercase",marginBottom:5},
  stalkerCard:{fontSize:19,color:C.gold,letterSpacing:1},stalkerCount:{fontSize:11,color:C.textDim,marginTop:5},
  ...
  sectionHeaderAction:{background:"none",border:"none",color:C.gold,fontSize:12,cursor:"pointer",letterSpacing:1},
  ...
  entryCardSpread:{fontSize:13,color:C.gold,letterSpacing:.5},entryCardDate:{fontSize:10,color:C.textDim,letterSpacing:1},
  ...
  cardBack:{width:132,height:204,background:"linear-gradient(145deg,#2d1f4a,#1a1030)",border:`2px solid ${C.goldDim}`,borderRadius:14,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",gap:14,cursor:"pointer",boxShadow:`0 8px 32px #00000060, inset 0 1px 0 #e2b96a20`},
  cardBackInner:{fontSize:44,color:C.goldDim},cardBackHint:{fontSize:10,letterSpacing:3,color:C.textDim,textTransform:"uppercase"},
  drawnCard:{background:"linear-gradient(160deg,#2a1e42,#201530)",border:"2px solid",borderRadius:14,padding:"22px 18px",marginBottom:16,textAlign:"center",boxShadow:"0 4px 24px #00000040"},
  drawnCardName:{fontSize:21,letterSpacing:2,color:C.text,margin:"10px 0"},drawnCardKeywords:{fontSize:11,color:C.goldDim,letterSpacing:1.5,marginBottom:12},
  ...
  slotPanelNum:{width:28,height:28,borderRadius:"50%",background:"linear-gradient(135deg,#3a2855,#2a1e42)",border:`1px solid ${C.goldDim}`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:13,color:C.gold,flexShrink:0},
  ...
  slotCard:{background:C.bgCard,border:`1px solid ${C.border}`,borderRadius:12,padding:"15px",marginBottom:12},
  slotLabel:{fontSize:14,color:C.gold,letterSpacing:1,marginBottom:3},slotMeaning:{fontSize:11,color:C.textDim,fontStyle:"italic"},
  ...
  slotDetailExpanded:{background:C.bgCard,border:`1px solid ${C.gold}40`,borderRadius:12,padding:"15px",marginBottom:12,boxShadow:`0 0 12px ${C.gold}15`},
  ...
  cardSearchInput:{width:"100%",background:C.bgInput,border:"none",borderBottom:`1px solid ${C.border}`,padding:"11px 13px",fontSize:13,color:C.text,outline:"none",boxSizing:"border-box"},
  ...
  spreadPickName:{fontSize:15,color:C.gold,letterSpacing:1,marginBottom:5},spreadPickPositions:{fontSize:11,color:C.textDim},
  ...
  savedBanner:{textAlign:"center",color:C.gold,fontSize:13,letterSpacing:2,padding:"16px",background:"linear-gradient(135deg,#2a1e0a20,#1e1628)",border:`1px solid ${C.goldDim}`,borderRadius:12,marginTop:10},
  ...
  filterBtnActive:{background:"linear-gradient(135deg,#3a2855,#2a1e42)",borderColor:C.goldDim,color:C.gold},
  ...
  cardGridName:{fontSize:10,color:C.textMid,textAlign:"center",lineHeight:1.3},cardGridCount:{fontSize:9,color:C.gold,marginTop:2},
  cardGridEdited:{position:"absolute",top:5,right:7,fontSize:9,color:C.goldDim},
  cardDetailHero:{border:"1px solid",borderRadius:14,padding:"26px 16px",textAlign:"center",marginBottom:18,background:"linear-gradient(160deg,#2d2048,#1e1530)"},
  cardDetailName:{fontSize:23,letterSpacing:3,color:C.text,fontWeight:400,margin:"10px 0 5px"},cardDetailNum:{fontSize:13,color:C.textDim,letterSpacing:3},
  cardDetailCount:{fontSize:11,color:C.gold,marginTop:7,letterSpacing:1},
  ...
  tabBtnActive:{background:"linear-gradient(135deg,#3a2855,#2a1e42)",color:C.gold},
  cardStandard:{background:C.bgCard,border:`1px solid ${C.border}`,borderRadius:12,padding:"18px"},
  cardKeywords:{fontSize:12,color:C.gold,letterSpacing:1.5,marginBottom:12,textAlign:"center"},
  cardMeaningText:{fontSize:14,color:C.textMid,lineHeight:1.8,fontStyle:"italic",margin:0},cardMine:{display:"flex",flexDirection:"column",gap:9},
  ...
  tag:{background:"linear-gradient(135deg,#3a2855,#2a1e42)",border:`1px solid ${C.goldDim}`,borderRadius:14,padding:"4px 12px",fontSize:11,color:C.gold},
  ...
  textarea:{width:"100%",background:C.bgInput,border:`1px solid ${C.border}`,borderRadius:9,padding:"11px 13px",fontSize:13,color:C.text,outline:"none",resize:"vertical",fontFamily:"'Cormorant Garamond',Georgia,serif",lineHeight:1.7,boxSizing:"border-box"},
  fieldLabel:{fontSize:10,letterSpacing:3,color:C.textDim,textTransform:"uppercase",display:"block",marginBottom:5},
  btnPrimary:{width:"100%",background:"linear-gradient(135deg,#3d2860,#2a1e42)",border:`1px solid ${C.goldDim}`,borderRadius:10,padding:"14px",fontSize:13,color:C.gold,cursor:"pointer",letterSpacing:2,textTransform:"uppercase",marginTop:5},
  ...
  settingsRowActions:{display:"flex",gap:12},settingsRowBtn:{background:"none",border:"none",color:C.gold,fontSize:12,cursor:"pointer",letterSpacing:.5},
  emptyState:{textAlign:"center",color:C.border,fontSize:12,letterSpacing:2,padding:"28px 0",textTransform:"uppercase"},
  modalOverlay:{position:"fixed",inset:0,background:"rgba(10,6,18,0.85)",zIndex:100,display:"flex",alignItems:"center",justifyContent:"center",padding:24},
  modalBox:{background:C.bgCard,border:`1px solid ${C.border}`,borderRadius:14,padding:"24px 20px",maxWidth:340,width:"100%"},
  ...
  alreadyPulledBox:{background:"linear-gradient(160deg,#2a1e42,#201530)",border:`2px solid ${C.goldDim}`,borderRadius:14,padding:"22px 18px",textAlign:"center"},
  alreadyPulledLabel:{fontSize:10,letterSpacing:3,color:C.textDim,textTransform:"uppercase",marginBottom:8},
  alreadyPulledCard:{fontSize:21,letterSpacing:2,color:C.gold,marginBottom:6},alreadyPulledDate:{fontSize:11,color:C.textDim,letterSpacing:1},
